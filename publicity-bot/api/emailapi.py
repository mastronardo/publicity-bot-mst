...
class EmailAPI:
    ...
    def __init__(self, host, port, email_from, emails_to, emails_cc, subject, body, password):
        ...
        self.__emails_to = ','.join(emails_to)
        self.__emails_cc = ','.join(emails_cc)
        self.__emails_subject = subject.replace("\n","").strip()
    ...
    def __send_message(self, client, email_from, to, message):
        ...
        try:
            ...
            logging.info('email successfully sent to %s ... completed' % (to))
        except Exception as e:
            logging.error('sending email to %s ... failed because %s' % (to, e))
